(function() {
  ...
  document.head.insertAdjacentHTML('beforeend', `<style>
    ...
    /* ===== STAGGERED CASCADE (IN & OUT) ===== */
    .reveal-target { opacity: 0; transform: translateY(40px); transition: opacity 0.7s cubic-bezier(0.16,1,0.3,1), transform 0.7s cubic-bezier(0.16,1,0.3,1); transition-delay: calc(var(--index, 0) * 0.08s); will-change: opacity, transform; }
    .reveal-target.is-visible { opacity: 1; transform: translateY(0); }
    .reveal-target.is-leaving { opacity: 0; transform: translateY(-24px); }
    .gallery-grid img { opacity: 0; transform: translateY(30px); transition: opacity 0.7s cubic-bezier(0.16,1,0.3,1), transform 0.7s cubic-bezier(0.16,1,0.3,1); transition-delay: calc(var(--index, 0) * 0.08s); will-change: opacity, transform; }
    ...
    /* soundtrack player strip */
    .track-strip { display: flex; align-items: center; gap: 18px; border-top: 1px solid #26262b; border-bottom: 1px solid #26262b; padding: 14px 0; opacity: 0.55; transition: opacity 0.6s ease, border-color 0.6s ease; }
    .track-strip.is-active { opacity: 1; border-color: var(--accent); }
    .track-strip .track-play { width: 34px; height: 34px; flex: 0 0 34px; border: 1px solid #3a3a42; background: transparent; color: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 14px; transition: all 0.25s ease; border-radius: 0; }
    .track-strip.is-active .track-play { border-color: var(--accent); color: var(--accent); }
    .track-strip .track-play:hover { background: var(--accent); border-color: var(--accent); color: #08080a; }
    .track-strip .track-meta { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
    .track-strip .track-label { font-family: 'SFMono-Regular','Menlo','Consolas',monospace; font-size: 10px; letter-spacing: 0.28em; text-transform: uppercase; color: #7a7a82; }
    .track-strip .track-name { font-size: 15px; font-weight: 700; letter-spacing: 0.02em; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .track-strip .track-eq { display: flex; align-items: flex-end; gap: 3px; height: 16px; margin-left: auto; opacity: 0; transition: opacity 0.4s ease; }
    .track-strip.is-playing .track-eq { opacity: 1; }
    .track-strip .track-eq span { width: 3px; background: var(--accent); height: 60%; animation: eq 0.9s ease-in-out infinite; }
    .track-strip .track-eq span:nth-child(2) { animation-delay: 0.25s; height: 100%; }
    .track-strip .track-eq span:nth-child(3) { animation-delay: 0.5s; height: 45%; }
    @keyframes eq { 0%,100% { transform: scaleY(0.5); } 50% { transform: scaleY(1); } }
    @media (prefers-reduced-motion: reduce) { .track-strip .track-eq span { animation: none; } }

    @media (max-width: 767px) {
      .track-strip .track-name { max-width: 55vw; }
    }
  </style>`);
  ...
  const gameData = [
    {
      id: "cWBdELprqqk", start: 26, accent: "#e53935", track: "Pragmata — Main Theme",
      title: "PRAGMATA",
    ...
    {
      id: "nTUoIyTMw0Q", start: 88, accent: "#f39c12", track: "James Bond: First Light — Main Theme",
    ...
    {
      id: "YHhwdyWkwTQ", start: 26, accent: "#8e24aa", track: "Crimson Desert — Main Theme",
    ...
    {
      id: "2VaLOc1FpSo", start: 57, accent: "#1e88e5", track: "Clair Obscur: Expedition 33 — Main Theme",
    ...
    {
      id: "4aVoaSixc0E", start: 169, accent: "#d81b60", bloodRain: true, track: "Stellar Blade: Blood Rain — Original Score",
    ...
    {
      id: "lTHTfqPTQ1k", start: 35, accent: "#6a1b9a", track: "Control: Resonant — Main Theme",
    ...
    {
      id: "BoHg3zeUSWI", start: 36, accent: "#00acc1", track: "Spine — Cyberpunk Underscore",
      title: "SPINE",
    ...
    {
      id: "avpTgTNadh4", start: 216, accent: "#c62828", track: "Stranger Than Heaven — Showman Suite",
    ...
    {
      id: "SB3siHVCHpY", start: 21, accent: "#2e7d32", track: "The Witcher 3: Sons of the Past — Lets Play",
  ...
  cards.forEach((card, index) => {
    ...
    if (vc) vc.insertAdjacentHTML('afterend', `
        <div class="track-strip" data-track="${index}" style="max-width:1200px;margin:0 auto;width:100%;">
